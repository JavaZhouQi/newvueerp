...
<template>
  <div class>
      ...
      <div style="margin-top: 15px;width:380px;float: left;">
        <el-input placeholder="请输入内容" v-model="selectValue" class="input-with-select">
          <el-select v-model="select" slot="prepend" placeholder="请选择">
            ...
            <el-option label="客户等级编号" value="levelID"></el-option>
            <el-option label="客户等级名称" value="levelName"></el-option>
    ...
    <el-table ref="filterTable" :data="tableData" style="width: 100%;margin-top:10px;">
      <el-table-column prop="levelID" label="客户等级编号" sortable width="180" column-key="date"></el-table-column>
      <el-table-column prop="levelName" label="客户等级名称" width="180"></el-table-column>
      ...
      <el-table-column label="操作" width="280">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="danger" @click="del(scope.row.levelID)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <page-helper @jumpPage="jumpPage" :page-number="currentPage" :totalCount="pagenumber"></page-helper>

    <el-dialog title="编辑客户等级" :visible.sync="addDialog" width="30%" :before-close="handleClose">
      <span>
        <el-form :model="entity" label-position="left" label-width="120px" status-icon :rules="rules">
          <el-form-item label="客户等级编号" prop="levelID">
            <el-input v-model="entity.levelID" :disabled="updatebool"></el-input>
          </el-form-item>
          <el-form-item label="客户等级名称" prop="levelName">
            <el-input v-model="entity.levelName"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    findPage() {
      ...
      request({
        url:
          "/comcustomerlevel/findPage?current=" +
          this.currentPage +
          "&size=" +
          this.currentSize,
        method: "post",
    ...
    //保存后新增
    saveAddition(){
      var number = this.entity.levelID
    },
    // 修改
    update(entity){
    ...
    del(id){
      request({
        url: "/comcustomerlevel/del?id="+id,
        method: "get"
      }).then(result => {
        Message.success(result.data.data)
        this.findPage()
