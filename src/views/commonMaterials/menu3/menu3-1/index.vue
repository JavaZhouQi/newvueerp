...
<template>
  <div class>
      ...
      <div style="margin-top: 15px;width:380px;float: left;">
        <el-input placeholder="请输入内容" v-model="selectValue" class="input-with-select">
          <el-select v-model="select" slot="prepend" placeholder="请选择">
            ...
            <el-option label="客户类型编号" value="classID"></el-option>
            <el-option label="客户类型名称" value="className"></el-option>
    ...
    <el-table ref="filterTable" :data="tableData" style="width: 100%;margin-top:10px;">
      <el-table-column prop="classID" label="客户类型编号" sortable width="180" column-key="date"></el-table-column>
      <el-table-column prop="className" label="客户类型名称" width="180"></el-table-column>
    ...
    <el-dialog title="编辑客户类型" :visible.sync="addDialog" width="30%" :before-close="handleClose">
      <span>
        <el-form :model="entity" label-position="left" label-width="120px" status-icon :rules="rules">
          <el-form-item label="客户类型编号" prop="classID">
            <el-input v-model="entity.classID" :disabled="updatebool"></el-input>
          </el-form-item>
          <el-form-item label="客户类型名称" prop="className">
...
export default {
  ...
  methods: {
    ...
    del(id){
      request({
        url: "/comcustclass/del?id="+id+"&flag=1",
        method: "get"
      }).then(result => {
        Message.success(result.data.data)
        this.findPage()
