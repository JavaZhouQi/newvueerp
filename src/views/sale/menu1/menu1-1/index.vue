...
<template>
  <div class>
      ...
      <div style="margin-top: 15px;width:380px;float: left;">
        <el-input placeholder="请输入内容" v-model="selectValue" class="input-with-select">
          <el-select v-model="select" slot="prepend" placeholder="请选择">
            ...
            <el-option label="销售订单类型编号" value="typeId"></el-option>
            <el-option label="销售订单类型名称" value="typeName"></el-option>
    ...
    <el-table ref="filterTable" :data="tableData" style="width: 100%;margin-top:10px;">
      <el-table-column prop="typeId" label="销售订单类型编号" sortable width="180" column-key="date"></el-table-column>
      <el-table-column prop="typeName" label="销售订单类型名称" width="180"></el-table-column>
    ...
    <el-dialog title="编辑销售订单类型" :visible.sync="addDialog" width="30%" :before-close="handleClose">
      <span>
        <el-form :model="entity" label-position="left" label-width="140px" status-icon :rules="rules">
          <el-form-item label="销售订单类型编号" prop="typeId">
            <el-input v-model="entity.typeId" :disabled="updatebool"></el-input>
          </el-form-item>
          <el-form-item label="销售订单类型名称" prop="typeName">
...
export default {
  ...
  methods: {
    ...
    findPage() {
      ...
      this.findData.flag=4;
      request({
        url:
          "/commontype/findPage?current=" +
          this.currentPage +
          "&size=" +
          this.currentSize,
        method: "post",
    ...
    del(id){
      request({
        url: "/commontype/del?id="+id+"&flag=4",
        method: "get"
      }).then(result => {
        Message.success(result.data.data)
        this.findPage()
