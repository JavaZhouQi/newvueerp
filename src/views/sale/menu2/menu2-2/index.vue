...
<template>
  <div class>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="单况" >
            <el-select v-model="smlordbillmain.billStatus" >
              <el-option label="未结案" :value="0" :disabled="isWriter"></el-option>
              <el-option label="已结案" :value="1" :disabled="isWriter"></el-option>
              <el-option label="无效" :value="2" :disabled="isWriter"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          ...
          <el-tab-pane label="内容">
            ...
            >
              ...
              <vxe-table-column
                ...
                width="100"
              ></vxe-table-column>
              <vxe-table-column field="preInDate" title="预出库日期" width="240" >
                  <template slot-scope="scope">
                      <el-date-picker v-model="scope.row.preInDate" size="mini" value-format="yyyy-MM-dd" @change="changeDate2(scope.row)" :disabled="isWriter"  type="date" placeholder="选择预出库日期" ></el-date-picker>
                  </template>
              </vxe-table-column>
              <vxe-table-column
                field="sumQty"
        ...
        <el-col :span="3">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item :disabled="isWriter"  @click.native="zhuandan">销售报价单转入</el-dropdown-item>
    ...
    <el-dialog title="多选--物料主文件设定" :visible.sync="dialogMaterials" width="35%">
     ...
     <el-dialog title="单选--销售报价单" :visible.sync="dialogQuote" width="35%">
      <span>
        <el-row :gutter="10">
          <el-col :span="24" style="padding:3px;height:48px;margin-bottom: 5px;">
            <el-form :inline="true" class="demo-form-inline">
              <el-form-item>
                <el-select v-model="union3.coumn">
                  <el-option
                    v-for="entity,index in union3.coumnList"
                    :label="entity.name"
                    :value="entity.value"
                    :key="index"
                  ></el-option>
                </el-select>
                <el-input-number v-model="union3.num" :min="1" :max="10" style="width:130px;"></el-input-number>
              </el-form-item>
            </el-form>
          </el-col>
          <el-col :span="5" style="height:200px;">
            <div>
              <ul style="height:200px;">
                <li v-for="coumn in union3.list" @click="selectCoumn3(coumn)">{{ coumn }}</li>
              </ul>
            </div>
          </el-col>
          <el-col :span="19" style="padding:3px;height:200px;">
            <el-table
              ref="singleTable"
              :data="union3.tableData"
              highlight-current-row
              @current-change="handleQuoteChange"
              style="width: 100%"
              height="200"
            >
              <el-table-column
                v-for="entity in union3.coumnList"
                :property="entity.value"
                :label="entity.name"
                width="120"
                :key="entity.value"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogQuote = false">取 消</el-button>
        <el-button type="primary" @click="dialogQuote = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      union3: {
        ...
        // 查询的条件
        selectName: {},
        tableName: "smlordbillmain", // 表名
        num: 1, // 截取数量
        list: [],
        coumn: "billNo", // 查询的列名 写name（名称）就可以了
        tableData: []
      },
      testnum:{},
      options:[],
  ...
  watch: {
    ...
    //客户选择
    "union3.coumn": function() {
      this.findByCoumnAndSize3();
    },
    "union3.num": function() {
      this.findByCoumnAndSize3();
    },
  ...
  methods: {
    ...
    query_one2(val) {
      request({
        ...
        data: val
          ...
          var main=result.data.data;
        for (let index = 0; index < main.subList.length; index++) {
            main.subList[index].tranType="销售报价单";
            main.subList[index].fromNO=main.subList[index].billNo
            main.subList[index].sumQty=main.subList[index].quantity;
        }
        main.billNo=this.smlordbillmain.billNo;
        main.billDate=this.smlordbillmain.billDate;
        this.smlordbillmain=main;
        this.smlordbillmain.billStatus=0;
        this.smlordbillmain.flag=2;
        this.smlordbillmain.customerName=this.smlordbillmain.comcustomer.id+"         "+this.smlordbillmain.comcustomer.fullName;
        for (let index = 0; index < this.options.length; index++) {
            // alert(this.smlordbillmain.departID+"                    "+this.options[index].comdepartment.departID);
    ...
    changeDate2(val) {
        if(val.preInDate!=undefined||val.preInDate!=""){
            this.$confirm('以下预出库日 都是用同一个日期吗', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              for (let index = 0; index < this.smlordbillmain.subList.length; index++) {
                this.smlordbillmain.subList[index].preInDate=val.preInDate;
              }
            }).catch(() => {
    ...
    findByCoumnAndSize3() {
      request({
        url:
          ...
          this.union3.num +
          "&table=" +
          this.union3.tableName,
        method: "get"
      }).then(result => {
        this.union3.list = result.data.data;
        this.union3.selectName[this.union3.coumn] = result.data.data[0];
        this.findTable3();
      });
    },
    // 查询条件
    selectCoumn3(coumn) {
      this.union3.selectName[this.union3.coumn] = coumn;
      this.findTable3();
    },
    // 查询数据详情
    findTable3() {
      this.union3.selectName.flag = 1;
      request({
        url: "/" + this.union3.tableName + "/findByTable",
        method: "post",
        data: this.union3.selectName
      }).then(result => {
        this.union3.tableData = result.data.data;
        for (let index = 0; index < this.union3.tableData.length; index++) {
            this.union3.tableData[index].billDate=this.timestampToTime(this.union3.tableData[index].billDate);
        }
        // console.log(this.union3.tableData);
        this.union3.selectName = {};
      });
